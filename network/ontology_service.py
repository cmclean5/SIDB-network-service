...
class ontology(object):
    ...
    def load(self, name='hp', pref='HP', rels='subClassOf', rm_obs_terms=True):
        ...
        if self.OBOname in LOCALname:
            ...
            #self.ont = self.ont.subontology(relations=self.OBOrels)
            #self.gg  = self.ont.get_graph()
    ...
    def insert_model_template(self, edge_key_type=0, rm_obs_terms=True):
        ...
        if self.gg is not None:
            ...
            for i, (nid, properties) in enumerate(t_gg.nodes(data=True)):
                ...
                #self.gg.add_node(i) #new code
                #for nk, nv in att_map.items():
                #    self.gg.node[i][nk] = nv
            ...
            for i, (eid_so, eid_si, properties) in enumerate(t_gg.edges(data=True)):
                ...
                #self.gg.add_edge(U,V) #new code
                #for ek, ev in att_map.items():
                #    self.gg[U][V][edge_key_type][ek] = ev
                #    clip = None

    def root_nodes(self, prefix=None, relations=None):

        if prefix is None:
            prefix = self.ONTpref

        if relations is None:
            relations = self.OBOrels

        roots = self.ont.get_roots(relations=relations, prefix=prefix)
        return roots

    def get_clip_ontology(self,subset=None,minimal=False,relations=None):
       #g    = self.ont.get_graph()
       #clip = get_minimal_subgraph(g, subset)
        if relations is None:
           relations = self.OBOrels

        if subset is not None and self.ont is not None:
           temp = self.ont.subontology(nodes=subset,minimal=minimal,relations=relations)
           self.clip = temp.get_graph()

        return self.clip

    def get_node_levels(self, node_dict=None, min_level=0, max_level=10, relations=None):
        ...
        for l in range(min_level,max_level):
            nlevel = self.ont.get_level(level=l, relations=relations)
            for k in set(nlevel):
                if k in node_dict:
                    node_dict[k] = l
