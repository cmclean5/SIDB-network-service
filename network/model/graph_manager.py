...
class Graph(object):
    ...
    def convertNodeIDs2Ints(self, firstID=0, ordering="default", label_attribute="term"):
        if self.graph is not None:
            self.graph = nx.convert_node_labels_to_integers(self.graph,first_label=firstID,ordering=ordering, label_attribute=label_attribute)

    def checkNodeIDlabel(self) -> bool:

        keys = self.find_node_att_keys()
        if 'id' in keys:
            return True
        else:
            return False
    ...
    def remap_node_property2(self, old_property, new_property):
        """
        Remap the value in node `old_property` attribute, i.e. 'label' with value from node `new_property` attribute, i.e. 'desc'
        ...
        if old_property in keys:
            mapping = {}
            for node_id in self.graph.nodes():
                node = self.graph.node[node_id]
                if old_property in node:
                    mapping[node_id] = node[old_property]
                    self.graph.node[node_id].pop(old_property,None)

            nx.set_node_attributes(self.graph, values = mapping, name = new_property)

    def add_node_property(self, new_prop_dict=None, search_label=None, new_label=None):

        if new_prop_dict is None:
            return
        ...
        for node_id in self.graph.nodes():
            node = self.graph.node[node_id]
            if search_label in node:
                mapping[node_id] = new_prop_dict.get(node[search_label])
            elif search_label is None:
                mapping[node_id] = new_prop_dict.get(node_id)

        nx.set_node_attributes(self.graph, values=mapping, name=new_label)
    ...
    def find_edge_att_keys(self):
        ...
        if self.graph.number_of_edges() > 0:
            for k1,k2,v in self.graph.edges(data=True):
    ...
    def flatten_property_values(self, graph=None, edge_type_key=0, sep=';'):
        ...
        if graph is not None:
            ...
            for i, (nid, properties) in enumerate(graph.nodes(data=True)):
                ...
                for k, v in properties.items():
                    if not isinstance(v, dict):
                        temp[k] = utils.get_values(values=v, sep=sep)

                ##self.graph.add_node(nid,attr_dict=temp) #old
                self.graph.add_node(nid)
                for nk, nv in temp.items():
                    self.graph.node[nid][nk] = nv
            ...
            for eid, (eso, esi, properties) in enumerate(graph.edges(data=True)):
                ...
                self.graph.add_edge(eso,esi)
                for ek, ev in temp.items():
                    self.graph[eso][esi][edge_type_key][ek] = ev
